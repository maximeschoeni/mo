class Stories {
  ...
  static async getCountryFile(story) {
    const countryId = story.country && story.country[0];
    if (countryId) {
      const country = await fetch(`/get/countries/${countryId}`).then(response => response.json());
      return country && this.coutryFiles.find(file => file.id === country.image);
    }
  }
    ...
    return {
      ...
      init: async div => {
        ...
        if (season && date1 && date2) {
          ...
          const [gameGroup] = await fetch(`/query/gameGroups?season=${seasonId}&date1=${date1}&date2=${date2}`).then(response => response.json());

          if (gameGroup) {

            this.stories = await fetch(`/query/stories?gameGroup=${gameGroup.id}`).then(response => response.json());
          ...
          this.files = await fetch(`/query/files?folder=stories`).then(response => response.json());

          console.log(this.files);

          this.coutryFiles = await fetch(`/query/files?folder=countries`).then(response => response.json());

          console.log(this.coutryFiles);
        ...
        // this.screensaverFiles = await fetch(`/query/files?folder=screensaver`).then(response => response.json());
        this.screensaverFiles = await fetch(`/query/files?ids=${(this.options.screensaver || []).join(",")}`).then(response => response.json());
      },
      children: [
        {
          class: "screensaver",
            ...
            screensaver.children = [
              {
                ...
                update: slides => {
                  ...
                  if (this.screensaving) {
                    this.currentScreenSaverIndex++;
                    if (this.currentScreenSaverIndex >= this.screensaverFiles.length) {
                      this.currentScreenSaverIndex = 0;
                    }
                    slides.children = this.screensaverFiles.map((file, index) => {
                      return {
                        class: "slide",
                        child: {
                          tag: "img",
                        ...
                        update: slide => {
                          const z = (this.screensaverFiles.length + this.currentScreenSaverIndex - index)%this.screensaverFiles.length;
                          slide.element.style.zIndex = z;
                          slide.element.classList.toggle("top", z === this.screensaverFiles.length-1);
                        }
                      };
                    });
                    setTimeout(() => {
                                                  ...
                                                  children: [
                                                    {
                                                      ...
                                                      child: {
                                                        tag: "img",
                                                        init: async img => {
                                                          const file = await this.getCountryFile(story);
                                                          if (file) {
                                                            img.element.src = "/uploads/" + file.filename;
                                                          }
                                                        }
