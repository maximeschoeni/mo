...
KarmaFieldsAlpha.field.collection = class extends KarmaFieldsAlpha.field.form {
  ...
  constructor(resource) {
    ...
    this.selectionBuffer = new KarmaFieldsAlpha.Buffer("state", this.resource.id, "selection");
  ...
  toggleOrder(key, newOrder = "asc") {
    ...
    let order = KarmaFieldsAlpha.Nav.get("order") || this.resource.params.order;
    let orderby = KarmaFieldsAlpha.Nav.get("orderby") || this.resource.params.orderby;
  ...
  async add(params = {}, index = 0) {
    ...
    for (let key in defaults) {
      ...
      this.buffer.change([defaults[key]], null, id, key);
